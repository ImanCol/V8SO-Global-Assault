...
public class Coprocessor
{
    ...
    {
        TruncateAndSetMACAndIR(accumulator.ir1 * accumulator.ir0 + ((long)mathsAccumulator.mac1 << (int)shift), 1, shift, lm);
        TruncateAndSetMACAndIR(accumulator.ir2 * accumulator.ir0 + ((long)mathsAccumulator.mac2 << (int)shift), 2, shift, lm);
        TruncateAndSetMACAndIR(accumulator.ir3 * accumulator.ir0 + ((long)mathsAccumulator.mac3 << (int)shift), 3, shift, lm);
        PushRGBFromMAC();
    }
