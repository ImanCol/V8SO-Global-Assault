...
public class VigTerrain : MonoBehaviour
{
    ...
    void Start()
    {
        newVertices = new Vector3[10242];
        newColors = new Color32[10242];
        newUVs = new Vector2[10242];
        newTriangles = new int[17][];
        newTriangles[0] = new int[10242];
        index2 = new int[16];

        /*for (int i = 1; i < newTriangles.Length; i++)
            newTriangles[i] = new int[4095];*/

        newTriangles[1] = new int[10242];
    ...
    public void FUN_1C910()
    {
        float angleX = LevelManager.instance.defaultCamera.transform.eulerAngles.x;
        float angleY = LevelManager.instance.defaultCamera.transform.eulerAngles.y;
    ...
    {
        if ((int)x > 0 && (int)z > 0 && (int)x < 0x7000000 && (int)z < 0x7000000)
        {
            uint uVar1 = x >> 16; //r10
            uint uVar2 = z >> 16; //r11
    ...
    public TileData GetTileByPosition(uint x, uint z)
    {
        if ((int)x > 0 && (int)z > 0 && (int)x < 0x7000000 && (int)z < 0x7000000)
        {
            ...
            int chunkID = chunks[id];
            return tileData[tiles[chunkID * 4096 + uVar1]];
        }
        else return tileData[0];
    }
    ...
    public Vector3Int FUN_1BB50(int param1, int param2)
    {
        ...
        if (param1 > 0 && param2 > 0 && param1 < 0x7000000 && param2 < 0x7000000)
        {
    ...
    public void FUN_2D16C(int param1, int param2, ref VigTransform param3)
    {
        ...
        SStack8 = FUN_1B998((uint)param1, (uint)param2);
        SStack8 = Utilities.VectorNormal(SStack8);
        param3.position.x = param1;
        iVar1 = FUN_1B750((uint)param1, (uint)param2);
        param3.position.y = iVar1;
        param3.position.z = param2;
        param3.rotation = Utilities.FUN_2A5EC(SStack8);
    }
        ...
        if ((_tileData[iVar9].flags & 1) == 0)
        {
            ...
            if (0 < (int)uVar5)
            {
                ...
                if ((_tileData[iVar9].flags & 2) == 0)
                {
					...
					Vector3 vector3 = terrainWorld[param4] / tFactor;
					newVertices[index] = vector3;
					...
					//{
					Vector3 vector31 = terrainWorld[param4 + 6] / tFactor;
					newVertices[index] = vector31;
                        newVertices[index + 1] = terrainWorld[param4 + 1] / tFactor;
                        newVertices[index + 2] = terrainWorld[param4 + 5] / tFactor;
                        newColors[index] = clrVar4;
                        newColors[index + 1] = clrVar6;
